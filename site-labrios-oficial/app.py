...
# CONFIGURAÇÃO CLOUDINARY (Apenas para imagens)
cloudinary.config(
...
class Member(db.Model):
    ...
    committee_type = db.Column(db.String(50)) # "Gestor" ou "Cientifico"
...
class LabSettings(db.Model):
    ...
    regimento_data = db.Column(db.LargeBinary) # PDF NO BANCO [4]
...
def home():
    ...
    cientificos = Member.query.filter_by(committee_type="Cientifico").all()
...
@app.route("/how-to-use") # Nome da rota mantido para compatibilidade, template alterado
def how_to_use():
...
def request_reservation(equipment_id):
    ...
    if request.method == "POST":
        new_res = Reservation(
            ...
            name=request.form.get("name"), institution=request.form.get("institution"),
            role=request.form.get("role"), lattes=request.form.get("lattes"),
...
@login_required
def admin_panel():
    return render_template("admin.html", settings=get_settings(), rules=Rule.query.all(),
                           members=Member.query.all(), equipments=Equipment.query.all(),
...
def update_settings():
    ...
    if pdf and pdf.filename != '':
        s.regimento_data = pdf.read() # Salva binário no banco [5]
...
def add_equipment():
    ...
    db.session.add(Equipment(name=request.form.get("name"), brand=request.form.get("brand"), model=request.form.get("model"),
                             purpose=request.form.get("purpose"), image=img_url, quantity=request.form.get("quantity", type=int)))
    db.session.commit()
    return redirect(url_for("admin_panel"))
...
def add_member():
    ...
    img_url = cloudinary.uploader.upload(f, folder="labrios/members")['secure_url'] if f and f.filename != '' else None
    db.session.add(Member(name=request.form.get("name"), role=request.form.get("role"), lattes=request.form.get("lattes"),
                          photo=img_url, committee_type=request.form.get("committee_type")))
    db.session.commit()
    return redirect(url_for("admin_panel"))
