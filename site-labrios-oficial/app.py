...
class Member(db.Model):
    ...
    photo = db.Column(db.String(255)) # URL Cloudinary
...
class LabSettings(db.Model):
    ...
    regimento_pdf = db.Column(db.String(255)) # URL Cloudinary
...
# -----------------------------
# ROTAS PÚBLICAS (Sem alterações estruturais)
# -----------------------------
@app.route("/")
def home():
...
def add_equipment():
    ...
    img_url = None
    if f and f.filename != '':
        upload_result = cloudinary.uploader.upload(f, folder="labrios/equipamentos")
...
def add_member():
    ...
    img_url = None
    if f and f.filename != '':
        upload_result = cloudinary.uploader.upload(f, folder="labrios/equipe")
        img_url = upload_result['secure_url']
...
# Rotas de Regras e Reservas permanecem as mesmas
